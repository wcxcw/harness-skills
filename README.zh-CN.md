...
这个 Skill Pack 的核心用途，是为项目创建并维护一套 Agent Harness 环境：

- 生成项目级 `AGENTS.md` 和 `harness/` 工作区
- 兼容新项目、已有代码库和已有 harness
- 把想法、规格、计划、执行记录、验证和评估统一收敛到 harness 流程中

Harness 维护是 agent-assisted：每次 run 之后，Agent 可以基于执行记录和用户确认的决策提出或执行小范围更新；它不会后台自动升级。
