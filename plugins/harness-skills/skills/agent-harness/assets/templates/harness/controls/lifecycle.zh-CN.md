...
没有 active run 时，微小改动不要创建 run。它必须同时满足：请求明确、影响局部、
不改变行为、不需要决策、能用一次小检查验证、可以局部回滚。例子包括错别字、
一句文案、一个明确选择器的字号修改、一个间距/颜色 token。宽泛视觉优化、整体
字体层级调整、跨页面修改、行为变化，或需要设计/产品/技术决策的任务不属于微小改动。
