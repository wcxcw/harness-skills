...
| Controls | 本地门槛 | [harness/controls/gates.md](harness/controls/gates.md) |
| Controls | 闭环生命周期 | [harness/controls/lifecycle.md](harness/controls/lifecycle.md) |
| Controls | Skill 兼容 | [harness/controls/skills.md](harness/controls/skills.md) |
...
| Scripts | Run 门槛检查 | [harness/scripts/check_run.py](harness/scripts/check_run.py) |

## 初始化模式

- `greenfield`：新项目或一句话想法。先读 `project-brief.md`、`initialization-notes.md`，再读当前 run 的 `workflow.md`、`design.md`、`spec.md`、`plan.md`。
...
- 如果用户用中文描述项目，`workflow.md`、`design.md`、`spec.md`、`plan.md`、run 记录和 harness 上下文默认使用中文。
...
新功能、greenfield 或产品方向使用 `design.md`。`full` run 或风险足够高时使用 `review.md`。`idea.md` 只用于 workflow 层出现前的旧 run。
...
当项目只有一句话想法、产品形态不清、技术栈未定，或用户希望先讨论方向时，不要直接写应用代码。使用 `brainstorming` 提问并生成 `design.md`。

用户回答后，先更新 `project-brief.md` 和 `initialization-notes.md`，再生成 `workflow.md`、`design.md`、`spec.md` 和 `plan.md`。如果仍有关键问题未确认，先把它作为 `plan.md` 的第一个非编码任务。
...
可以使用 workflow skills，但前提是它们能帮助满足本地 spec、plan、gate 或验证要求。Workflow skills 不能替代本仓库的 harness。

使用 [harness/controls/skills.md](harness/controls/skills.md) 将 workflow skills 映射到本地阶段。如果外部 skill 与项目命令、guardrails 或用户指令冲突，以本地 harness 为准；冲突重要时先询问。

## Agent 工作流

1. 从 `harness/context/initialization-notes.md` 判断初始化模式。
2. 根据用户语言和项目文档语言确定本次输出语言。
3. 新项目先读 `project-brief.md`，再读当前 run 的 `workflow.md`、`design.md`、`spec.md` 和 `plan.md`。
4. 老项目先读需要的 `harness/context/` 文件，再读现有项目文档和配置，最后读当前 run。
5. 阅读 `harness/controls/lifecycle.md`、`gates.md` 和 `skills.md`。
6. 只加载完成任务所需的上下文。
7. 编辑前检查 controls 和 guardrails。
8. 编辑应用代码前确认没有未解决的阻塞问题。
9. 当存在 active run 时，运行实现前门槛。
10. 一次执行一个小任务。
11. 运行已确认的验证命令。
12. 声称完成前，把证据记录到当前 run。
13. 创建或更新 `review.md`。
14. 当存在 active run 时，运行交付前门槛。
15. 交付前更新 evaluation。
16. 检查 `Context Updates`、`execution-log.md`、`review.md` 和 `evaluation.md` 是否暴露了 harness 缺口。
17. 只有在用户要求维护 harness、当前流程允许，或发现仓库事实性错误时，才更新规范 harness 文件；否则列出建议等待确认。
